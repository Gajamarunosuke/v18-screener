...
def send_discord(webhook_url: str, results: list[dict]) -> None:
    ...
    if not results:
        content = f"**V18スクリーナー {today}**\n本日の候補なし"
    else:
        top = sorted(results, key=lambda x: x["dist_m"])[:25]
        rows = [f"{'コード':<6} {'終値':>6} {'MA距離':>6} {'近接':<4}"]
        rows.append("-" * 32)
        for r in top:
            rows.append(f"{r['code']:<6} {r['close']:>6.0f} {r['dist_m']:>5.2f}% {r['near']:<4}")
        if len(results) > 25:
            rows.append(f"...他{len(results)-25}銘柄")

        content = "\n".join([
            f"**V18スクリーナー {today} {run_time}** — **{len(results)}銘柄**ヒット",
            "```",
            *rows,
            "```",
            "📊 詳細: https://docs.google.com/spreadsheets/d/1XUGNkIo6TbInJQOHf6obZzUhJkJ3MyzvkUDCTqAll_U",
        ])
